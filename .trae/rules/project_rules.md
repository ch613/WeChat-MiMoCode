...
**只有当对项目源代码进行了实质性修改（新增、编辑、删除代码文件）时，才执行 git 提交并推送。**

纯问答、解释、文档阅读等非代码修改操作不需要 git 同步。

### 执行步骤

对源代码进行修改后，在项目根目录 `f:\program\wechat-mimocode` 执行：
...
当确实修改了代码时，即使是很小的改动（改一行注释、修一个 typo）也要提交。频繁的小提交比一次大提交更利于回退。
